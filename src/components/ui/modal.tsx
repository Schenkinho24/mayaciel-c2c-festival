  ...
  return (
    <Fragment>
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4 animate-scale-in">
        <div 
          className={cn(
            "bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-auto animate-enter",
            className
          )}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center justify-between p-4 border-b border-tequila-secondary">
            {title && <h2 className="text-xl font-semibold text-tequila-dark">{title}</h2>}
            <Button 
              variant="ghost" 
              size="icon" 
              onClick={onClose}
              className="ml-auto text-tequila-dark hover:bg-tequila-neutral"
            >
              <X className="h-5 w-5" />
